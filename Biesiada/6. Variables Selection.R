...
# 1. VARIABLES TO EXCLUDE FROM THE MODEL ---------------------------------------
...
# 2. CREATE PRELIMINARY MODEL DATA----------------------------------------------
...
# 7. Correlations End Model

# Boolean Correlations
boolean_data_end <- model_data_end |>
  select(
    where(is.logical),
    where(~ is.factor(.x) && nlevels(.x) == 2)
  ) |>
  mutate(
    # TRUE/FALSE -> 1/0
    across(
...
boolean_correlations_end <- cor(
  boolean_data_end,
  use = "pairwise.complete.obs",
  method = "pearson"
)

view(boolean_correlations_end )

# Numeric Correlations

# Take only the columns that can be correlated:
#    numeric stays numeric, TRUE/FALSE becomes 1/0.
#    IDs and the outcome are left out.
cor_data <- model_data_end |>
  select(-any_of(c("match_id", "player_id", "team_id", "PTR"))) |>
  mutate(across(where(is.logical), as.integer)) |>
  select(where(is.numeric))

# Drop constant columns (they have no variation -> no correlation)
constant_cols <- names(cor_data)[sapply(cor_data, \(x) length(unique(na.omit(x))) <= 1)]
print(constant_cols)          # good to know what they were
cor_data <- cor_data |> select(-any_of(constant_cols))

#  The correlation matrix (Spearman = rank-based, safe for skewed data)
cor_mat <- cor(cor_data, use = "pairwise.complete.obs", method = "spearman")

#  Turn it into a simple pair list, strongest first, each pair once
cor_pairs <- as.data.frame(as.table(cor_mat)) |>
  rename(var1 = Var1, var2 = Var2, r = Freq) |>
  filter(as.character(var1) < as.character(var2)) |>
  arrange(desc(abs(r)))

#  The pairs that need a decision (|r| >= 0.7 -> keep one of the two)
high <- cor_pairs |> filter(abs(r) >= 0.7)
print(high, row.names = FALSE)
...
# Saving 
saveRDS(
  model_data_start_gain,
  "model_data_start_gain502.rds")
